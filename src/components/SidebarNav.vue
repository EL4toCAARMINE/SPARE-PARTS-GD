...
<template>
  <aside class="h-screen bg-[#2f3b46] text-white shadow-xl transition-all duration-200" :class="opened ? 'w-64' : 'w-32'">
    <div class="flex items-center gap-3 px-4 py-4 border-b border-white/10">
      ...
      <button class="ml-auto opacity-80 hover:opacity-100" @click="opened = !opened"><span class="material-icons">menu</span></button>
    </div>
        ...
        <li v-for="(item, i) in filteredMenu" :key="i">
          ...
          <details v-else class="group">
            <summary class="flex items-center gap-3 px-3 py-2 rounded-md hover:bg-white/10 cursor-pointer list-none">
              ...
              <span v-if="opened" class="material-icons ml-auto transform group-open:rotate-180 transition">expand_more</span>
            </summary>
